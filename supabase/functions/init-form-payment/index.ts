// Supabase Edge Function: init-form-payment
// Subscription checkout for KoboDocs Form (starter/pro/business), monthly
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!plan || !PLAN_PRICE_KOBO[plan]) {
      return new Response(JSON.stringify({ error: "plan must be one of: starter, pro, business" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const amountKobo = PLAN_PRICE_KOBO[plan];
    const orderReference = `kbd_form_${crypto.randomUUID()}`;
    ...
    const squadRes = await fetch(`${SQUAD_BASE_URL}/transaction/initiate`, {
      ...
      body: JSON.stringify({
        ...
        callback_url: `${Deno.env.get("SITE_URL") ?? "https://kobodocs.com.ng"}/forms/app/`,
      }),
    });
