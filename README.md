...
- **Self-serve checkout** (added same day, once the payment-policy reversal covered Booking too): `/booking/` pricing buttons call `init-booking-payment` via the shared `assets/subscribe.js` helper (same pattern as Forms/Survey), and `squad-webhook` gained a `kobodocs_booking` branch that upserts `booking_subscriptions` on successful payment. `/admin/forms-survey-plans/` (now covering all three products) still exists as the manual-override fallback.
- **⚠️ Known open issue to verify**: `squad-webhook` currently has temporary debug logging (`webhook_debug_log` table) specifically because recent `cv_builder`/`kobodocs_form`/`kobodocs_survey` payments were seen *not* granting entitlements reliably. The `kobodocs_booking` branch added here uses the exact same upsert pattern as the form/survey branches, so it should work once that's resolved — but it inherits the same open question rather than being independently verified. **Test an actual Booking checkout end-to-end before relying on it**, and check `webhook_debug_log` if a payment doesn't grant a plan.
