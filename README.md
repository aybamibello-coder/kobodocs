...
| `/survey/` — KoboDocs Survey | **Live.** Builder (8 question types: text, paragraph, multiple choice, checkboxes, rating, NPS, Likert, ranking), skip logic (same show-if mechanism as Forms), AI survey generator (Pro+), multilingual starter templates (English/Pidgin in full; Hausa/Yoruba/Igbo titles only, flagged for native-speaker review), publish/close, public fill page, responses table + CSV export. Same plan-limit pattern as Forms. Not yet built: cross-tab analytics, matrix questions, self-serve plan upgrades. |
...
- Not yet built: e-signature *envelope* workflow (multi-signer, sequential — the signature field here is a single inline capture, not the full `/esign/` engine), cross-tab analytics, matrix questions, self-serve plan upgrades.
