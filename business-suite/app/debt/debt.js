      ...
      return `
        <div class="debt-row" data-doc="${d.id}">
          <div>
            ...
            <div class="debt-meta">${invNumber} · Due ${fmtDate(d.due_date)}${d.amount_paid > 0 ? ` · ${naira(d.amount_paid)} paid so far` : ''}</div>
          </div>
          <div style="display:flex; align-items:center; gap:16px; flex-wrap:wrap;">
            <div class="debt-amount">${naira(d.balance)}</div>
            <div class="debt-actions">
              <button class="reminder-btn primary" data-action="record-payment">Record payment</button>
      ...
      row.querySelectorAll('button').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          if (action === 'record-payment') {
            const amountStr = prompt(`Amount received for ${(doc.data && doc.data.invNumber) || 'this invoice'} (balance: ${naira(doc.balance)}):`);
            if (!amountStr) return;
            const amountNum = Number(String(amountStr).replace(/,/g, ''));
            if (!amountNum || amountNum <= 0) { toast('Enter a valid amount.'); return; }

            const newAmountPaid = Number(doc.amount_paid || 0) + amountNum;
            ...
            const { error: updateErr } = await supabase.from('documents')
              ...
              .eq('id', doc.id);
            if (updateErr) { toast('Could not record payment: ' + updateErr.message); return; }

            const { error: eventErr } = await supabase.from('document_payments').insert({
              document_id: doc.id,
