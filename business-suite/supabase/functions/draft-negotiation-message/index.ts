...
Deno.serve(async (req: Request) => {
  ...
  const [{ data: receivables }, { data: promises }, { data: notes }, { data: openDisputes }, { data: openDocRequests }] = await Promise.all([
  ...
  const blockedCount = (receivables ?? []).filter((rv: any) => disputedReceivableIds.has(rv.id) || blockedByDocReceivableIds.has(rv.id)).length;

  const items = (receivables ?? [])
    .filter((rv: any) => !disputedReceivableIds.has(rv.id) && !blockedByDocReceivableIds.has(rv.id)) // don't ask for payment on items blocked by a dispute or missing document
    .map((rv: any) => {
  ...
  if (!items.length) {
    return ok({
      error: blockedCount > 0
        ? "This client's outstanding balance is entirely under dispute or blocked by a missing document. Resolve that first before requesting payment."
  ...
  const facts = {
    ...
    last_note: lastNote,
    has_separate_disputed_items: blockedCount > 0,
  };
...
- total_balance_naira already EXCLUDES any disputed or document-blocked items \u2014 do not mention or reference a dispute/missing document in the message even if has_separate_disputed_items is true, since that is being handled separately.
